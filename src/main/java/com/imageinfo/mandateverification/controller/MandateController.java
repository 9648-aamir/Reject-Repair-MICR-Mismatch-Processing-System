package com.imageinfo.mandateverification.controller;
...
import com.imageinfo.mandateverification.dao.MandateVerificationDao;
...
public class MandateController extends SelectorComposer<Component> {
	...
	private static final String SELECT_BATCH = "selectedBatch";
	...
	private void populateBatchGrid() {
		...
		for (MandateVerification batch : batches) {
			...
			row.appendChild(new Label(String.valueOf(batch.getPendingCount())));
			row.appendChild(new Label(String.valueOf(batch.getProcessedCount())));
			...
			row.addEventListener("onClick", event -> {
				...
				Sessions.getCurrent().setAttribute(SELECT_BATCH, batch);
	...
	public void onDebtorIfscMicrEntered() throws SQLException {
		MandateVerification mav = (MandateVerification) Sessions.getCurrent().getAttribute(SELECT_BATCH);
	...
	public void displayMandateImage() {
		MandateEntryDao mandateEntryDao = new MandateEntryDao();
		try {
			MandateVerification mav = (MandateVerification) Sessions.getCurrent().getAttribute(SELECT_BATCH);
			if (mav == null) {
			...
			byte[] imgBytes = mandateEntryDao.getMandateImage(mav.getBatchId());
			if (imgBytes == null) {
	...
	private void updateZoom() {
		myImage.setStyle("transform: scale(" + currentScale + "); transition: transform 0.2s ease;");
	}
	
	public void clear() {
	    utilityCode.setSelectedItem(null);
	    scanDate.setSelectedItem(null);
	    mandateType.setSelectedItem(null);
	    actiontype.setSelectedItem(null);
	}
}
