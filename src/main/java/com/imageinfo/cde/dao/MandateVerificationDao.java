...
public class MandateVerificationDao {
	...
	private static final String SCAN_CODE = "scan_date";

	public List<String> getUtilityCodes() {
		List<String> utilityCodes = new ArrayList<>();
		String sql = "SELECT DISTINCT utility_code FROM mandate_verification ORDER BY utility_code";
	...
	public List<String> getScanDates(String utilityCode) {
		...
		String sql = "SELECT DISTINCT scan_date FROM mandate_verification WHERE utility_code = ?  ORDER BY scan_date DESC";
		try (Connection conn = DataBaseConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setString(1, utilityCode);
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					scanDates.add(rs.getDate(SCAN_CODE).toString());
				}
			}
		} catch (Exception e) {
	...
	public List<MandateVerification> getBatches(String utilityCode, String scanDate, String mandateType,
		...
		String sql = """
				SELECT batch_id,
				    ...
				    SUM(CASE WHEN status = 'processed' THEN 1 ELSE 0 END) AS accepted_count,
				    SUM(CASE WHEN status = 'send back' THEN 1 ELSE 0 END) AS rejected_count,
				    status,
				    processed_by
		...
		try (Connection conn = DataBaseConnection.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					mv.setScanDate(rs.getDate(SCAN_CODE).toLocalDate());
					mv.setBatchCount(rs.getInt("batch_count"));
					mv.setAcceptCount(rs.getInt("accepted_count"));
					mv.setRejectedCount(rs.getInt("rejected_count"));
		...
		try (Connection con = DataBaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					...
					entry.setScanDate(rs.getDate(SCAN_CODE) != null ? rs.getDate(SCAN_CODE).toLocalDate() : null);
